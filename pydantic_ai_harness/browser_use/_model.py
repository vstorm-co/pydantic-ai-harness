...
class PydanticAIChatModel(BaseChatModel):
    ...
    async def ainvoke(
        ...
        if output_format is None:
            text_result = await Agent(self._model).run(None, message_history=history)
            return ChatInvokeCompletion(completion=text_result.output, usage=_map_usage(text_result.usage))
        result = await Agent(self._model, output_type=output_format).run(None, message_history=history)
